...
class RealityLogAnalyzer:
    ...
    def standardize_id(self, val):
        ...
        s = s.replace("LIKE", "").replace("FD", "").replace("FP", "").strip()
        s = re.sub(r'(?<=\d)[A-Z]+$', '', s)
        return s
    ...
    def get_stock_data(self):
        ...
        for _, row in df.iterrows():
            ...
            if (b.startswith('2G') or b.startswith('8G')) and b.endswith('C'):
                grade = "C"; join_key = b[2:-1] if b.startswith('2G') or b.startswith('8G') else b
            elif b.startswith('2G') and b.endswith('8'):
                grade = "B"; join_key = b[2:-1]
            elif b.startswith('2G'):
                grade = "AX"; join_key = b[2:]
            elif b.startswith('2F'):
                grade = "A"; join_key = b[2:]
            else:
                grade = "A"; join_key = b # 預設
    ...
    def get_reality_tasks(self):
        ...
        for _, row in df.iterrows():
            ...
            if is_status_only:
                ...
                join_key = batch_raw[2:] if batch_raw.startswith('2F') else batch_raw
                if join_key in stock_data:
                    t['stock'] = stock_data[join_key]
        ...
        # 將支撐天數注入回每個機台的 poy_list 中以便前端表格顯示
        for t in tasks.values():
            for p in t['poy_list']:
                pid = p['clean_id']
