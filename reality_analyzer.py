...
class RealityLogAnalyzer:
    ...
    def get_reality_tasks(self):
        ...
        # 準備累計庫存基礎資料
        # 注意：我們將直接修改 stock_data 中的數值，因此需要先深拷貝或確保不影響原始 map (這裡直接用)
        ...
        def is_valid_machine(m):
            m = str(m).upper(); return False if (not m or m == 'NAN' or re.search(r'M0[1-8]', m)) else True
        df['m_name'] = df.iloc[:, 1].astype(str).str.strip().str.upper()
        # 這裡執行機台名稱轉換，確保 S01 對接到計畫的 S1
        df['m_name'] = df['m_name'].replace({'S01': 'S1', 'S02': 'S2'})
