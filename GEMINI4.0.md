...
1.  **`RealityLogAnalyzer` (多源引擎 - reality_analyzer.py)**：
    ...
    *   **精準狀態判定 (Precision Status)**：
        *   **數據源**：直接讀取生產資訊「夜班」欄位（X 欄為 L 側，Y 欄為 R 側）。
        *   **邏輯映射**：
            *   若儲存格為**數字**：判定為「正常運行」，隱藏異常字眼。
            *   若儲存格為**空值/0**：判定為「停機」。
            *   若儲存格為**文字**：直接呈現（如 `SAMPLE`, `TEST`, `改紡`）。
    *   **機台結構適應**：
        *   **單列模式**：自動判別並標註 `L邊停機`、`R邊停機` 或 `全台停機`。
        *   **雙列模式**：針對 T05 等跨行機台，精確對接 L 列看 X 欄、R 列看 Y 欄，解決並存衝突。
    *   **特殊狀態支援**：系統會自動識別 AA 欄位的特殊字眼，並在儀表板呈現，同時在累計庫存計算中自動排除此類非正式產量。
